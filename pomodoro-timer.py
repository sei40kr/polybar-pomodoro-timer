...
class Status:
    ...
    def show(self):
        if self.status == "work":
            if self.active:
                prefix = WORK_ACTIVE_PREFIX
                suffix = WORK_ACTIVE_SUFFIX
            else:
                prefix = WORK_INACTIVE_PREFIX
                suffix = WORK_INACTIVE_SUFFIX
        else:
            if self.active:
                prefix = BREAK_ACTIVE_PREFIX
                suffix = BREAK_ACTIVE_SUFFIX
            else:
                prefix = BREAK_INACTIVE_PREFIX
                suffix = BREAK_INACTIVE_SUFFIX

        sys.stdout.write("{}{}{}\n".format(prefix, self.timer, suffix))
        sys.stdout.flush()
